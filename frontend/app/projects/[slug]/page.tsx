...
export default function ProjectDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <PageLayout>
      <div className="pt-14">
        <section className="relative py-20 px-6 border-b border-border overflow-hidden bg-navy/20">
          ...
          <div className="relative max-w-5xl mx-auto">
            ...
            <h1 className="font-display font-bold text-4xl md:text-6xl text-foreground tracking-tight leading-tight mb-4">
              {project.title}
            </h1>
            {project.institution && (
            ...
            <div className="mb-16 pb-16 border-b border-border">
              ...
              <div className="flex flex-wrap gap-2">
                {project.techStack.map((tech) => (
                  <span key={tech} className="font-mono text-sm text-foreground/80 bg-surface border border-border px-3 py-2 hover:border-accent/40 transition-colors duration-200 shadow-sm">
                    {tech}
                  </span>
                ))}
              </div>
            ...
            <div className="mb-16 pb-16 border-b border-border">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border border-border divide-x divide-y divide-border shadow-sm">
                {project.highlights.map((h, i) => (
                  <div key={h} className="group p-5 bg-surface hover:bg-surface-raised transition-colors duration-200">
              ...
              <div className="border border-border bg-surface shadow-sm">
                <div className="flex flex-col divide-y divide-border">
                  {project.futureGoals.map((g, i) => (
                    <div key={g} className="group flex items-start gap-5 p-6 hover:bg-surface-raised transition-colors duration-200">
